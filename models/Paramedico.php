...
class Paramedico {
    ...
    //Funcion para obtener datos del Paramedico
    public function obtenerParamedico() {
        $query = $this->db->query("SELECT p.id,p.identidad,p.primer_nombre,p.segundo_nombre,p.primer_apellido,p.segundo_apellido,p.genero,p.estado_civil,p.fecha_nacimiento,p.direccion,p.telefono,p.fecha_ingreso,d.id as iddepartamento, d.descripcion as departamento,c.id as idciudad,c.descripcion as ciudad, e.descripcion as estado FROM departamento as d,paramedico as p, estado as e, ciudad as c WHERE c.id_departamento=d.id AND p.id_estado = e.id AND p.id_ciudad = c.id AND (p.id_estado=3 OR p.id_estado=5 ) ORDER BY p.id ASC ");
        return $query;
    }
    
    
        //Funcion para obtener datos del paramedico filtrado por la ciudad
    public function getParamedico($idciudad,$estado ) {
        $query = $this->db->query("SELECT p.id,p.identidad,p.primer_nombre,p.segundo_nombre,p.primer_apellido,p.segundo_apellido,p.genero,p.estado_civil,p.fecha_nacimiento,p.direccion,p.telefono,p.fecha_ingreso FROM paramedico as p, ciudad as c WHERE p.id_ciudad=c.id AND p.id_ciudad=$idciudad AND p.id_estado=$estado ORDER BY p.id ASC");
        return $query;
    }

    //Funcion para obtener del Paramedico a modificar
    public function modificar() {

        $paramedico = $this->db->query("SELECT p.id,p.identidad,p.primer_nombre,p.segundo_nombre,p.primer_apellido,p.segundo_apellido,p.genero,p.estado_civil,p.fecha_nacimiento,p.direccion,p.telefono,p.fecha_ingreso,d.id as iddepartamento, d.descripcion as departamento,c.id as idciudad,c.descripcion as ciudad, e.descripcion as estado FROM departamento as d,paramedico as p, estado as e, ciudad as c WHERE c.id_departamento=d.id AND p.id_estado = e.id AND p.id_ciudad = c.id AND (p.id_estado=3 OR p.id_estado=5 ) AND p.id= '{$this->getId()}' ");

        return $paramedico->fetch_object();
    }

    //Funcion para insertar datos del Paramedico
    public function insertarParamedico() {
        $sql = "INSERT INTO paramedico VALUES(NULL,'{$this->getIdentidad()}','{$this->getPrimer_nombre()}','{$this->getSegundo_nombre()}','{$this->getPrimer_apellido()}','{$this->getSegundo_apellido()}','{$this->getFecha_nacimiento()}','{$this->getGenero()}','{$this->getEstado_civil()}','{$this->getTelefono()}','{$this->getDireccion()}','{$this->getId_ciudad()}','{$this->getId_estado()}','{$this->getFecha_ingreso()}')";

        $insertar = $this->db->query($sql);   //ejecucion del query para insertar datos del Paramedico
    ...
    public function eliminarParamedico() {
        $sql = "UPDATE paramedico SET id_estado = '4' WHERE id= '{$this->getId()}' ";
