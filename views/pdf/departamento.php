...
// Cabecera de página
    function Header() {
       ...
       // $this->Image('views/assets/img/logo.png', 10, 16, 30); //posicion de la imagen x, y z
        ...
        $this->SetXY(62, 20); //x, y posicion del titulo
        $this->cell(0, 10, "Atenciones por Departamento", 0, 1); //ancho, largo, contenido,salto de linea
        $this->SetFont('Times', 'B', 12); // tipo de letra, negrita, tamaño
        $this->SetXY(38, 40); //x, y posicion del titulo
        $this->cell(0, 10, "Fecha Inicio: ".$_POST['inicio'], 0, 1); //ancho, largo, contenido,salto de linea
        $this->SetXY(38, 50); //x, y posicion del titulo
...
//Encabezados
$pdf->SetX(40); //ancho, largo, contenido,salto de linea
...
while ($atencion = $atenciones->fetch_object()) {
    $ciudad = substr(utf8_decode($atencion->departamento), 0, 40);
    $pdf->SetX(40); //ancho, largo, contenido,salto de linea
    $pdf->Cell(10, 8, $i, 'B', 0, 'C', 1); //ancho, largo, contenido,borde,salto de linea,alineado, color
    $pdf->Cell(60, 8, utf8_decode($atencion->departamento), 'B', 0,'C', 1);//ancho, largo, contenido,borde,salto de linea,alineado, color
...
$pdf->Cell(70, 8, "Total", 'B',0,'C', 1); //ancho, largo, contenido,borde,salto de linea,alineado, color
$pdf->SetTextColor(255, 255, 255 ); // tipo de letra, negrita, tamaño
$pdf->Cell(60, 8, $total, 'B',1,'C', 1); //ancho, largo, contenido,borde,salto de linea,alineado, color
ob_get_clean();
$pdf->Output('', 'departamento.pdf');
?>