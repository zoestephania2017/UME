...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->SetXY(55, 20); //x, y posicion del titulo
        $this->cell(0, 10, "Atenciones por Centro Asistencial", 0, 1); //ancho, largo, contenido,salto de linea
...
$pdf->Cell(40, 8, "Departamento", 'B', 0, 'C', 1); //ancho, largo, contenido,borde,salto de linea,alineado, color
$pdf->Cell(40, 8, "Ciudad", 'B', 0, 'C', 1); //ancho, largo, contenido,borde,salto de linea,alineado, color
$pdf->Cell(60, 8, "Centro Asistencial", 'B', 0, 'C', 1); //ancho, largo, contenido,borde,salto de linea,alineado, color
$pdf->Cell(40, 8, "Atenciones", 'B', 1, 'C', 1); //ancho, largo, contenido,borde,salto de linea,alineado, color
...
while ($atencion = $atenciones->fetch_object()) {
    $departamento = substr(utf8_decode($atencion->departamento), 0, 25);
    $ciudad = substr(utf8_decode($atencion->ciudad), 0, 25);
    $centro = substr(utf8_decode($atencion->centro), 0, 25);
    $pdf->SetX(10); //ancho, largo, contenido,salto de linea
    $pdf->Cell(10, 8, $i, 'B', 0, 'C', 1); //ancho, largo, contenido,borde,salto de linea,alineado, color
    $pdf->Cell(40, 8, $departamento, 'B', 0, 'C', 1); //ancho, largo, contenido,borde,salto de linea,alineado, color
    $pdf->Cell(40, 8, $ciudad, 'B', 0, 'C', 1); //ancho, largo, contenido,borde,salto de linea,alineado, color
    $pdf->Cell(60, 8, $centro, 'B', 0, 'C', 1); //ancho, largo, contenido,borde,salto de linea,alineado, color
    $pdf->Cell(40, 8, $atencion->totales, 'B', 1, 'C', 1); //ancho, largo, contenido,borde,salto de linea,alineado, color
    $i++;
...
$pdf->Cell(150, 8, "Total", 'B', 0, 'C', 1); //ancho, largo, contenido,borde,salto de linea,alineado, color
$pdf->SetTextColor(255, 255, 255); // tipo de letra, negrita, tamaño
$pdf->Cell(40, 8, $total, 'B', 1, 'C', 1); //ancho, largo, contenido,borde,salto de linea,alineado, color
ob_get_clean();
$pdf->Output('', 'centroasistencial.pdf');
?>